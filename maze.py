...
def plot_action_values(action_values):
    ...
    fig.colorbar(tripcolor)
     ...
     # Draw grid lines for each state and action
    for i in range(6):  # Create 5x5 grid, so 6 lines horizontally and vertically
        ax.vlines(i, 0, 5, colors="black", linestyles="dotted", linewidth=1)
        ax.hlines(i, 0, 5, colors="black", linestyles="dotted", linewidth=1)
    # Draw cross lines within each cell to represent each action direction
    for row in range(5):
        for col in range(5):
            ax.plot([col, col + 1], [5 - row, 5 - (row + 1)], color="black", linestyle="--", linewidth=0.5)
            ax.plot([col, col + 1], [5 - (row + 1), 5 - row], color="black", linestyle="--", linewidth=0.5)
